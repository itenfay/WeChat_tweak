...
- (void)onDone:(UIBarButtonItem *)item {
    if (self.delegate && [self.delegate respondsToSelector:@selector(onMultiSelectGroupReturn:)]) {
        NSArray *result = self.wcpl_userDidChangeSelection ? [self wcpl_selectedUserNames] : [self wcpl_sanitizedUserNames:self.blackList];
...
- (void)wcpl_applySelectionsOnce {
    NSArray *userNames = self.blackList;
    ...
    for (NSString *contactName in userNames) {
        ...
        @try {
            ...
            WCPLLog(@"群聊预选 addSelect 失败: %@", userName);
        }
    }
