...
NSString *const kWCPLTopRightMenuMarkAllReadID = @"wcpl_mark_all_read";

BOOL WCPLMarkAllReadTopRightMenuEnabled(void) {
...
static id _Nullable wcpl_markAllRead_createLoadingView(UIView * _Nullable hostView,
    ...
    return WCPLLoadingViewCreateAndStart(hostView, text, YES);
}
...
static void wcpl_markAllRead_stopLoadingView(id _Nullable loadingView, BOOL ok, NSString * _Nullable text) {
    ...
    WCPLLoadingViewStopWithResult(loadingView, ok, text);
...
void WCPLMarkAllReadPresentConfirmAlert(void) {
    UIViewController *presenter = wcpl_markAllRead_topMostViewController();
    if (!presenter) {
        return;
    }
...
NSString * _Nullable WCPLTopRightMenuSafeItemIDFromSender(id _Nullable sender) {
    if (!sender) {
        return nil;
    }
    ...
    if (!data) {
        data = WCPLSafeValueForKey(sender, @"m_data");
    }

    NSString *itemID = WCPLSafeValueForKey(data, @"m_nsID");
    if (![itemID isKindOfClass:[NSString class]] || itemID.length == 0) {
        itemID = WCPLSafeValueForKey(sender, @"m_nsID");
    }
    if (![itemID isKindOfClass:[NSString class]] || itemID.length == 0) {
        return nil;
    }
...
UIImage * _Nullable WCPLTopRightMenuMarkAllReadIconImage(void) {
    ...
    dispatch_once(&onceToken, ^{
        NSString *svg =
            ...
            "</svg>";
        icon = WCPLIconImageFromSVGOrSystemSymbols(svg, @[@"checkmark.circle", @"envelope.open"], 18);
...
void WCPLTopRightMenuSyncMarkAllReadMenuItem(id _Nullable menuBtn) {
    if (!menuBtn) {
        return;
    }

    BOOL enabled = WCPLMarkAllReadTopRightMenuEnabled();
    id items = WCPLSafeValueForKey(menuBtn, @"m_dicItems");
    id data = WCPLSafeValueForKey(menuBtn, @"m_data");
    id showIDs = WCPLSafeValueForKey(data, @"m_arrShowIDs");
    ...
    if (existingItem) {
        ...
        if (!wcpl_topRightMenu_itemLooksDataObject(existingItem)) {
            id existingData = WCPLSafeValueForKey(existingItem, @"m_data");
...
BOOL WCPLTopRightMenuResolveShowIDResult(id _Nullable itemID, BOOL origResult) {
    if ([itemID isKindOfClass:[NSString class]] &&
        [(NSString *)itemID isEqualToString:kWCPLTopRightMenuMarkAllReadID]) {
        return WCPLMarkAllReadTopRightMenuEnabled();
    }
    return origResult;
}
