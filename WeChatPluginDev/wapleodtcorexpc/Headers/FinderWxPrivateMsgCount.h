//
// ClassDump By HuangBai Private
//  Copyright (C) 1997-2019 Steve Nygard. Updated HuangBai-2024 
//

#import <ProtobufLite/WXPBGeneratedMessage.h>

@class FinderRedDotCtrlInfo, NSString;

@interface FinderWxPrivateMsgCount : WXPBGeneratedMessage
{
}

+ (void)initialize;

// Remaining properties
@property(retain, nonatomic) FinderRedDotCtrlInfo *ctrlInfo; // @dynamic ctrlInfo;
@property(retain, nonatomic) NSString *latestNotificationBriefing; // @dynamic latestNotificationBriefing;
@property(nonatomic) unsigned int latestNotificationTime; // @dynamic latestNotificationTime;
@property(nonatomic) unsigned int newNotificationCount; // @dynamic newNotificationCount;

@end

